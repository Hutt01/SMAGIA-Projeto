...
class MedicationRobotAgent(Agent):
    ...
    class MessageReceiverBehaviour(CyclicBehaviour):
            ...
            if msg:
                ...
                if performative == "inform" and task_type == "delivery":
                    ...
                    try:
                        ...
                        if room:
                            self.agent.location = self.agent.room_locations.get(room)
                        else:
                    ...
                    availability_info = {
                        ...
                        "room": self.agent.room_locations.get(room) if room else None
                    }
        ...
        async def deliver_medication(self, task, room):
            self.agent.robot_status = RobotStatus.DELIVERING
            self.agent.location = self.agent.room_locations.get(room)
            print(f"[{self.agent.name}] New Status: Delivering. Heading to {room} at {self.agent.location}")

            # --- MQTT goal publish ---
            goal = self.agent.location
            topic = f"123/meia/{self.agent.robot_name.lower()}/goal"
            self.agent.mqtt_goal_succeeded.clear()  # Reset event
            self.agent.mqtt_client.publish(topic, json.dumps(goal))
            print(f"[{self.agent.name}] Published goal to {topic}: {goal}")

            # --- Wait for goal_succeeded ---
            ...
            if not self.agent.mqtt_goal_succeeded.is_set():
